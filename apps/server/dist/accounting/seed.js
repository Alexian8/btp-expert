...
exports.CATEGORY_PCG = exports.SEED_JOURNALS = exports.SEED_ACCOUNTS = void 0;
exports.seedChartOfAccountsMysql = seedChartOfAccountsMysql;
exports.seedJournalsMysql = seedJournalsMysql;
exports.SEED_ACCOUNTS = [
    // Classe 1
...
async function seedChartOfAccountsMysql(db) {
    for (const a of exports.SEED_ACCOUNTS) {
        await db.query(`INSERT IGNORE INTO chart_of_accounts
        (numero, libelle, classe, type, nature, parentNumero, isAuxiliary, isLocked)
       VALUES (?, ?, ?, ?, ?, '', ?, 1)`, [a.numero, a.libelle, a.classe, a.type, a.nature, a.isAuxiliary ?? 0]);
    }
}
async function seedJournalsMysql(db) {
    for (const j of exports.SEED_JOURNALS) {
        await db.query(`INSERT IGNORE INTO journals (code, libelle, type, defaultCompte, isLocked)
       VALUES (?, ?, ?, ?, 1)`, [j.code, j.libelle, j.type, j.defaultCompte]);
    }
}
