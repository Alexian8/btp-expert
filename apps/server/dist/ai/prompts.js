...
exports.AI_EXPENSE_CATEGORIES = void 0;
exports.buildQuoteLinePrompts = buildQuoteLinePrompts;
exports.sanitizeQuoteLineDescription = sanitizeQuoteLineDescription;
exports.buildExpenseCategoryPrompts = buildExpenseCategoryPrompts;
exports.parseExpenseCategory = parseExpenseCategory;
/**
...
function buildQuoteLinePrompts(args) {
    const system = "Tu es l'assistant d'un artisan du bâtiment français. " +
    ...
    const prompt = `Rédige la description d'une ligne de devis intitulée : « ${args.title.trim()} ». ` +
 ...
 */
function sanitizeQuoteLineDescription(raw) {
...
function buildExpenseCategoryPrompts(args) {
    const system = "Tu es l'assistant comptable d'un artisan du bâtiment français. " +
        "Tu classes les dépenses. Tu réponds toujours par UN SEUL MOT : la clé de catégorie, rien d'autre.";
    const keys = exports.AI_EXPENSE_CATEGORIES.map((c) => `${c.key} = ${c.label}`).join("\n");
    const supplier = (args.supplierName ?? "").trim();
    const prompt = `Clés de catégorie possibles :\n${keys}\n\n` +
 ...
 */
function parseExpenseCategory(raw) {
    ...
    for (const { key } of exports.AI_EXPENSE_CATEGORIES) {
        if (firstWord === key)
            return key;
    }
    ...
    const byLength = [...exports.AI_EXPENSE_CATEGORIES].sort((a, b) => b.key.length - a.key.length);
    ...
    for (const { key, label } of byLength) {
        if (text.includes(normalize(label)))
            return key;
    }
    return null;
}
