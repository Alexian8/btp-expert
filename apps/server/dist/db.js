...
async function runEnterpriseMigrations(db) {
  ...
  `);
    ...
    try {
        await db.query("UPDATE agenda_events SET startAt = `startDate` WHERE (startAt IS NULL OR startAt = '') AND `startDate` IS NOT NULL AND `startDate` != ''");
        await db.query("UPDATE agenda_events SET endAt = `endDate` WHERE (endAt IS NULL OR endAt = '') AND `endDate` IS NOT NULL AND `endDate` != ''");
        await db.query("UPDATE agenda_events SET allDay = isAllDay WHERE allDay = 0 AND isAllDay = 1");
    }
    catch (e) {
