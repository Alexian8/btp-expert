...
import { useChantierCategoriesStore, type ChantierCategory } from "@/stores/chantierCategoriesStore";
import {
  CHANTIER_CAT_ICON_OPTIONS, CHANTIER_CAT_COLOR_OPTIONS,
  getChantierCatIcon, getChantierCatColor,
} from "./chantierCategoryMeta";

// ═══════════════════════════════════════════════════════════════════════════
// ChantierCategoriesManager — Gestion des catégories chantier (Session 22)
// CRUD avec drag&drop, icône et couleur
// ═══════════════════════════════════════════════════════════════════════════
...
export function ChantierCategoriesManager({ open, onClose }: Props) {
  const { categories, fetch, create, update, remove, reorder } = useChantierCategoriesStore();
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-border shrink-0">
              <div>
                <h2 className="text-base font-semibold">Catégories de chantier</h2>
                <p className="text-xs text-muted-foreground">
                  Corps d'état · {categories.length} catégorie{categories.length > 1 ? "s" : ""} · glisser pour réordonner
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                  ...
                  {editingId === cat.id ? (
                    ...
                    <CategoryRow
                      ...
                      onDelete={async () => {
                        ...
                        if (r.success) {
                          if (r.detached && r.detached > 0) {
                            toast.success(`Catégorie supprimée. ${r.detached} chantier(s) détaché(s).`);
                          } else {
              ...
              {creating ? (
                <div className="border border-dashed border-primary rounded-lg">
                  <CategoryEditor
                    category={{ id: "", name: "", iconKey: "hammer", colorKey: "slate" } as any}
                    isNew
            ...
            <div className="p-3 border-t border-border bg-muted/20 text-[11px] text-muted-foreground">
              <p>
                💡 Les catégories servent à classer les chantiers par corps d'état
                (électricité, plomberie...) et à tagger les lignes de devis pour analyser
                la répartition du CA par poste.
              </p>
            </div>
          </motion.div>
        </motion.div>
...
function CategoryRow({
  ...
  category: ChantierCategory;
  onEdit: () => void;
  onDelete: () => void;
}) {
  const [confirmDel, setConfirmDel] = useState(false);
  const Icon = getChantierCatIcon(category.iconKey);
  const color = getChantierCatColor(category.colorKey);
...
function CategoryEditor({
  ...
  const [iconKey, setIconKey] = useState(category.iconKey || "hammer");
  ...
  const PreviewIcon = getChantierCatIcon(iconKey);
  const previewColor = getChantierCatColor(colorKey);
    ...
    <div className="p-3 space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <Input
            ...
            placeholder="Ex: Électricité, Plomberie, Toiture..."
            className="mt-1 h-8"
            autoFocus
          />
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-10 gap-1.5">
          {CHANTIER_CAT_ICON_OPTIONS.map((opt) => {
            const Icon = opt.Icon;
            return (
              <button
                key={opt.key}
      ...
      <div>
        ...
        <div className="grid grid-cols-12 gap-1.5">
          {CHANTIER_CAT_COLOR_OPTIONS.map((opt) => (
            <button
              key={opt.key}
