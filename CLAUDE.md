...
### IA locale (assistant embarqué, `/api/ai`)
Petit modèle GGUF exécuté **sur le serveur** (CPU) via `node-llama-cpp` (dépendance **optionnelle** de `@btp/server` — si le module ou le modèle manquent, le serveur démarre normalement et le service se déclare indisponible). Moteur dans `apps/server/src/ai/` : `engine.ts` (chargement paresseux du modèle `AI_MODEL_PATH`, import dynamique ESM-depuis-CJS, **une inférence à la fois** + `AI_THREADS=2` par défaut pour le fair-use CPU o2switch) et `prompts.ts` (prompts + parsing, logique pure testée ; ⚠️ duplique la liste `ExpenseCategory` car le serveur ne peut pas importer `@btp/types` à l'exécution). Routes (`routes/ai.ts`, auth + rate-limit dédié `RATE_LIMIT_AI_*`) : `GET /api/ai/status` (gating UI), `POST /suggest-quote-line`, `POST /categorize-expense`, `POST /test` (admin). Config : `AI_MODEL_PATH` (requis pour activer), `AI_THREADS`, `AI_CONTEXT_SIZE`, `AI_MAX_TOKENS`, `AI_TIMEOUT_MS`, `AI_QUEUE_MAX`. Modèle téléchargé via `apps/server/scripts/download-ai-model.js` (Qwen2.5-1.5B-Instruct Q4_K_M par défaut).

Côté front : types dans `packages/types/src/ai.ts`, méthodes **optionnelles** `aiStatus`/`aiSuggestQuoteLine`/`aiCategorizeExpense`/`aiTest` sur `window.btpAPI` (shim web → REST ; **preload desktop → stubs « indisponible »** : l'app desktop n'a pas de serveur, l'UI masque les boutons via `aiStore.useAiAvailable()`). Points d'entrée UI : bouton « Rédiger avec l'IA » sous la description de ligne (`QuoteItemsEditor`), « Suggérer avec l'IA » sur la catégorie de dépense (`ExpenseEditorModal`), diagnostic admin **Paramètres → IA locale** (`AiSection` : statut + test d'inférence). iOS : consommer les mêmes routes REST (écrans à faire).
...
- **IA locale** : desktop = stubs « indisponible » (pas de serveur à interroger — piste : brancher l'app desktop sur le REST du serveur ou embarquer node-llama-cpp dans le main Electron) ; iOS = routes REST disponibles mais **écrans non faits** ; qualité limitée par la taille du modèle (1.5B) et vitesse dépendante de la charge CPU du mutualisé.
